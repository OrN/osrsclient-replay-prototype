
// $FF: renamed from: lv
public class class_339 extends class_338 {

   // $FF: renamed from: j int
   public int field_3885;
   // $FF: renamed from: rz int
   static final int field_3886 = 5;
   // $FF: renamed from: r int
   public int field_3887;
   // $FF: renamed from: p int
   public int field_3888;


   // $FF: renamed from: ak (int, int) void
   void method_6015(int var1, int var2) {
      this.field_3888 = var1 * -245371139;
      this.field_3885 = var2 * -2116718473;
   }

   // $FF: renamed from: ao (int, int, int) void
   void method_6016(int var1, int var2, int var3) {
      try {
         this.field_3888 = var1 * -245371139;
         this.field_3885 = var2 * -2116718473;
      } catch (RuntimeException var4) {
         throw class_223.method_4281(var4, "lv.ao(" + ')');
      }
   }

   // $FF: renamed from: aq (int) int
   public int method_6017(int var1) {
      try {
         return 1693720149 * this.field_3888;
      } catch (RuntimeException var2) {
         throw class_223.method_4281(var2, "lv.aq(" + ')');
      }
   }

   // $FF: renamed from: aw (int) boolean
   public boolean method_6018(int var1) {
      try {
         return 1693720149 * this.field_3888 > 0;
      } catch (RuntimeException var2) {
         throw class_223.method_4281(var2, "lv.aw(" + ')');
      }
   }

   // $FF: renamed from: af (int, int) void
   void method_6019(int var1, int var2) {
      this.field_3888 = var1 * -836614076;
      this.field_3885 = var2 * -2116718473;
   }

   // $FF: renamed from: an () boolean
   public boolean method_6020() {
      return 1693720149 * this.field_3888 > 0;
   }

   // $FF: renamed from: ay () int
   public int method_6021() {
      return 1693720149 * this.field_3888;
   }

   // $FF: renamed from: <init> () void
   void method_6011() {
      try {
         super.method_6011();
         this.field_3888 = 245371139;
      } catch (RuntimeException var1) {
         throw class_223.method_4281(var1, "lv.<init>(" + ')');
      }
   }

   // $FF: renamed from: au () boolean
   public boolean method_6022() {
      return 1693720149 * this.field_3888 > 0;
   }

   // $FF: renamed from: aa () boolean
   public boolean method_6023() {
      return 1693720149 * this.field_3888 > 0;
   }
}
