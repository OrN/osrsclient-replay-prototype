
// $FF: renamed from: hj
public final class class_261 {

   // $FF: renamed from: c int
   int field_2905;
   // $FF: renamed from: n int
   int field_2906;
   // $FF: renamed from: l int
   int field_2907;
   // $FF: renamed from: y int
   int field_2908;
   // $FF: renamed from: r int
   int field_2909;
   // $FF: renamed from: f int
   int field_2910;
   // $FF: renamed from: j int
   int field_2911;
   // $FF: renamed from: b int
   int field_2912;
   // $FF: renamed from: d int
   int field_2913;
   // $FF: renamed from: s int
   int field_2914;
   // $FF: renamed from: p gl
   public class_91 field_2915;
   // $FF: renamed from: u int
   int field_2916;
   // $FF: renamed from: o long
   public long field_2917;
   // $FF: renamed from: v int
   int field_2918;
   // $FF: renamed from: aj int
   static final int field_2919 = 38;


   // $FF: renamed from: gj (byte) void
   static void method_4991(byte var0) {
      try {
         if(845336023 * client.field_1544 >= 0) {
            if(var0 == 4) {
               throw new IllegalStateException();
            }

            if(null != client.field_1605[845336023 * client.field_1544]) {
               if(var0 == 4) {
                  throw new IllegalStateException();
               }

               class_208.method_4144(client.field_1605[client.field_1544 * 845336023], false, 1977930716);
            }
         }

      } catch (RuntimeException var1) {
         throw class_223.method_4281(var1);
      }
   }

   // $FF: renamed from: <init> () void
   class_261() {
      super();
      try {
         this.field_2917 = 0L;
         this.field_2905 = 0;
      } catch (RuntimeException var1) {
         throw class_223.method_4281(var1);
      }
   }
}
