
// $FF: renamed from: gl
public abstract class class_91 extends class_59 {

   // $FF: renamed from: cj int
   public int field_911;


   // $FF: renamed from: cf (int, int, int, int, int, int, int, int, long) void
   void method_1606(int var1, int var2, int var3, int var4, int var5, int var6, int var7, int var8, long var9) {
      class_92 var11 = this.method_1609((byte)-128);
      if(var11 != null) {
         this.field_911 = var11.field_911 * 1;
         var11.method_1607(var1, var2, var3, var4, var5, var6, var7, var8, var9);
      }

   }

   // $FF: renamed from: ct (int, int, int, int, int, int, int, int, long) void
   void method_1607(int var1, int var2, int var3, int var4, int var5, int var6, int var7, int var8, long var9) {
      try {
         class_92 var11 = this.method_1609((byte)-16);
         if(var11 != null) {
            this.field_911 = var11.field_911 * 1;
            var11.method_1607(var1, var2, var3, var4, var5, var6, var7, var8, var9);
         }

      } catch (RuntimeException var12) {
         throw class_223.method_4281(var12, "gl.ct(" + ')');
      }
   }

   // $FF: renamed from: j () gr
   protected class_92 method_1608() {
      return null;
   }

   // $FF: renamed from: <init> () void
   protected class_91() {
      super();
      try {
         this.field_911 = -1901239416;
      } catch (RuntimeException var1) {
         throw class_223.method_4281(var1, "gl.<init>(" + ')');
      }
   }

   // $FF: renamed from: n (byte) gr
   protected class_92 method_1609(byte var1) {
      try {
         return null;
      } catch (RuntimeException var2) {
         throw class_223.method_4281(var2, "gl.n(" + ')');
      }
   }

   // $FF: renamed from: r () gr
   protected class_92 method_1610() {
      return null;
   }

   // $FF: renamed from: cq (int, int, int, int, int, int, int, int, long) void
   void method_1611(int var1, int var2, int var3, int var4, int var5, int var6, int var7, int var8, long var9) {
      class_92 var11 = this.method_1609((byte)-52);
      if(var11 != null) {
         this.field_911 = var11.field_911 * 1;
         var11.method_1607(var1, var2, var3, var4, var5, var6, var7, var8, var9);
      }

   }

   // $FF: renamed from: cr (int, int, int, int, int, int, int, int, long) void
   void method_1612(int var1, int var2, int var3, int var4, int var5, int var6, int var7, int var8, long var9) {
      class_92 var11 = this.method_1609((byte)-58);
      if(var11 != null) {
         this.field_911 = var11.field_911 * 1;
         var11.method_1607(var1, var2, var3, var4, var5, var6, var7, var8, var9);
      }

   }

   // $FF: renamed from: p () gr
   protected class_92 method_1613() {
      return null;
   }
}
