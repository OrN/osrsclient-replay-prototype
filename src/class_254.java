
// $FF: renamed from: gw
public class class_254 {

   // $FF: renamed from: y gw
   static final class_254 field_2868;
   // $FF: renamed from: n gw
   public static final class_254 field_2869;
   // $FF: renamed from: c int
   static final int field_2870 = 11;
   // $FF: renamed from: f gw
   public static final class_254 field_2871;
   // $FF: renamed from: p gw
   public static final class_254 field_2872;
   // $FF: renamed from: j java.lang.String
   public final String field_2873;
   // $FF: renamed from: v gw
   public static final class_254 field_2874;


   // $FF: renamed from: jb (io, byte) void
   static void method_4900(class_37 var0, byte var1) {
      try {
         if(var0.field_89 * 861366973 == client.field_1625 * -1275777925) {
            if(var1 <= 1) {
               throw new IllegalStateException();
            }

            client.field_1495[1408898685 * var0.field_240] = true;
         }

      } catch (RuntimeException var2) {
         throw class_223.method_4281(var2);
      }
   }

   // $FF: renamed from: <clinit> () void
   static {
      class_254 var10000 = new class_254("details");
      field_2874 = var10000;
      var10000 = new class_254("compositemap");
      field_2869 = var10000;
      var10000 = new class_254("compositetexture");
      field_2871 = var10000;
      var10000 = new class_254("area");
      field_2868 = var10000;
      var10000 = new class_254("labels");
      field_2872 = var10000;
   }

   // $FF: renamed from: <init> (java.lang.String) void
   class_254(String var1) {
      super();
      try {
         this.field_2873 = var1;
      } catch (RuntimeException var2) {
         throw class_223.method_4281(var2);
      }
   }
}
