
// $FF: renamed from: mw
public class class_27 {

   // $FF: renamed from: ff long
   public long field_9;
   // $FF: renamed from: fw mw
   public class_27 field_10;
   // $FF: renamed from: fj mw
   public class_27 field_11;


   // $FF: renamed from: fe () boolean
   public boolean method_101() {
      return this.field_11 != null;
   }

   // $FF: renamed from: fc () void
   public void method_102() {
      if(this.field_11 != null) {
         this.field_11.field_10 = this.field_10;
         this.field_10.field_11 = this.field_11;
         this.field_10 = null;
         this.field_11 = null;
      }
   }

   // $FF: renamed from: fz () boolean
   public boolean method_103() {
      return this.field_11 != null;
   }

   // $FF: renamed from: fa () void
   public void method_104() {
      if(this.field_11 != null) {
         this.field_11.field_10 = this.field_10;
         this.field_10.field_11 = this.field_11;
         this.field_10 = null;
         this.field_11 = null;
      }
   }

   // $FF: renamed from: ft () void
   public void method_106() {
      if(this.field_11 != null) {
         this.field_11.field_10 = this.field_10;
         this.field_10.field_11 = this.field_11;
         this.field_10 = null;
         this.field_11 = null;
      }
   }

   // $FF: renamed from: fs () boolean
   public boolean method_107() {
      return this.field_11 != null;
   }

   // $FF: renamed from: eq () void
   public void method_108() {
      if(this.field_11 != null) {
         this.field_11.field_10 = this.field_10;
         this.field_10.field_11 = this.field_11;
         this.field_10 = null;
         this.field_11 = null;
      }
   }

   // $FF: renamed from: fp () boolean
   public boolean method_109() {
      return this.field_11 != null;
   }
}
