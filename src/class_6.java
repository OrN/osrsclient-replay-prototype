
// $FF: renamed from: gi
public interface class_6 {

   // $FF: renamed from: g (int, int, int) int[]
   int[] method_10(int var1, int var2, int var3);

   // $FF: renamed from: z (nd) void
   void method_11(Buffer var1);

   // $FF: renamed from: b (fo) void
   void method_12(class_230 var1);

   // $FF: renamed from: y (int, int, int, int) int[]
   int[] method_13(int var1, int var2, int var3, int var4);

   // $FF: renamed from: f (int, int, byte) boolean
   boolean method_14(int var1, int var2, byte var3);

   // $FF: renamed from: j (nd, int) void
   void method_15(Buffer var1);

   // $FF: renamed from: v (fo, int) void
   void method_16(class_230 var1, int var2);

   // $FF: renamed from: d (int, int, int) boolean
   boolean method_17(int var1, int var2, int var3);

   // $FF: renamed from: s (int, int, int) boolean
   boolean method_18(int var1, int var2, int var3);

   // $FF: renamed from: u (int, int, int) boolean
   boolean method_19(int var1, int var2, int var3);

   // $FF: renamed from: l (int, int) boolean
   boolean method_20(int var1, int var2);

   // $FF: renamed from: o (int, int) boolean
   boolean method_21(int var1, int var2);

   // $FF: renamed from: c (int, int) boolean
   boolean method_22(int var1, int var2);

   // $FF: renamed from: n (int, int, int, int) boolean
   boolean method_23(int var1, int var2, int var3, int var4);

   // $FF: renamed from: e (int, int, int) int[]
   int[] method_24(int var1, int var2, int var3);

   // $FF: renamed from: a (int, int) ii
   class_280 method_25(int var1, int var2);

   // $FF: renamed from: k (int, int) ii
   class_280 method_26(int var1, int var2);

   // $FF: renamed from: m (int, int) ii
   class_280 method_27(int var1, int var2);

   // $FF: renamed from: x (nd) void
   void method_28(Buffer var1);

   // $FF: renamed from: p (int, int, int) ii
   class_280 method_29(int var1, int var2, int var3);
}
