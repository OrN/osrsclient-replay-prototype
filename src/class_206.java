import java.io.IOException;

// $FF: renamed from: ls
public abstract class class_206 {

   // $FF: renamed from: gt gt
   static class_251 field_2401;


   // $FF: renamed from: f (int) int
   public abstract int method_4117(int var1) throws IOException;

   // $FF: renamed from: y (int, int) boolean
   public abstract boolean method_4118(int var1, int var2) throws IOException;

   // $FF: renamed from: m (byte[], int, int) int
   public abstract int method_4119(byte[] var1, int var2, int var3) throws IOException;

   // $FF: renamed from: n (int) int
   public abstract int method_4120(int var1) throws IOException;

   // $FF: renamed from: p (byte[], int, int, int) int
   public abstract int method_4121(byte[] var1, int var2, int var3, int var4) throws IOException;

   // $FF: renamed from: x (byte[], int, int) int
   public abstract int method_4122(byte[] var1, int var2, int var3) throws IOException;

   // $FF: renamed from: v (int) void
   public abstract void method_4123(int var1);

   // $FF: renamed from: b () void
   public abstract void method_4124();

   // $FF: renamed from: d () void
   public abstract void method_4125();

   // $FF: renamed from: s () void
   public abstract void method_4126();

   // $FF: renamed from: u () int
   public abstract int method_4127() throws IOException;

   // $FF: renamed from: l () int
   public abstract int method_4128() throws IOException;

   // $FF: renamed from: k (int) boolean
   public abstract boolean method_4129(int var1) throws IOException;

   // $FF: renamed from: c () int
   public abstract int method_4130() throws IOException;

   // $FF: renamed from: e () int
   public abstract int method_4131() throws IOException;

   // $FF: renamed from: r (byte[], int, int, int) void
   public abstract void method_4132(byte[] var1, int var2, int var3, int var4) throws IOException;

   // $FF: renamed from: a (int) boolean
   public abstract boolean method_4133(int var1) throws IOException;

   // $FF: renamed from: g (int) boolean
   public abstract boolean method_4134(int var1) throws IOException;

   // $FF: renamed from: <init> () void
   protected class_206() {
      super();
      try {
      } catch (RuntimeException var1) {
         throw class_223.method_4281(var1, "ls.<init>(" + ')');
      }
   }

   // $FF: renamed from: z (byte[], int, int) int
   public abstract int method_4136(byte[] var1, int var2, int var3) throws IOException;

   // $FF: renamed from: o () int
   public abstract int method_4137() throws IOException;

   // $FF: renamed from: h (byte[], int, int) void
   public abstract void method_4138(byte[] var1, int var2, int var3) throws IOException;

   // $FF: renamed from: q (byte[], int, int) void
   public abstract void method_4139(byte[] var1, int var2, int var3) throws IOException;

   // $FF: renamed from: b (jv, jv, java.lang.String, java.lang.String, byte) kt
   public static class_69 method_4140(class_306 var0, class_306 var1, String var2, String var3, byte var4) {
      try {
         int var5 = var0.method_5399(var2, -397557984);
         int var6 = var0.method_5400(var5, var3, -1815968159);
         return class_306.method_5485(var0, var1, var5, var6, (byte)7);
      } catch (RuntimeException var7) {
         throw class_223.method_4281(var7, "ls.b(" + ')');
      }
   }

   // $FF: renamed from: v (int) int
   static int method_4141(int var0) {
      try {
         return 12;
      } catch (RuntimeException var1) {
         throw class_223.method_4281(var1, "ls.v(" + ')');
      }
   }
}
